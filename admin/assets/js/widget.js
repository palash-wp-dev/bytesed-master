;(function ($) {
    ...
    $(document).ready(function () {
        /***********************************
         irtech Custom widget js
         ************************************/
        $(document).on('widget-updated',function (e,widget) {
            var widget_id = $(widget).attr('id');
            if (widget_id.indexOf('irtech_about_us_widget') != 1){
                prefetch();
            }
        });

        $('body').off('click','.irtech_flogo_uploader');

        $('body').on('click','.irtech_flogo_uploader',function (e) {
            var el = this;
            ...
            file_uploder_frame.on('insert',function () {
                ...
                var img_prev_container = $(el).siblings('.irtech-logo-preview');
                ...
                $(el).siblings().children('.irtech_logo_id').val(selected_ids.join(","));
                $(el).siblings().children('.irtech_logo_id').trigger('change');
            ...
            file_uploder_frame.on('open',function () {
                ...
                console.log($(el).siblings().children('.irtech_logo_id').val());
                var atts = $(el).siblings().children('.irtech_logo_id').val().split(",");
        ...
        function prefetch () {
            $('.irtech_logo_id').each(function () {
                var attid = $(this).val();
                var container = $(this).parent().siblings('.irtech-logo-preview');
                container.html('');

                if ( attid ){
                    $(this).parent().parent().find('.irtech_flogo_uploader').val('Change Image');
