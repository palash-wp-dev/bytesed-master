<div class="irtech-product-extract-wrapper">
	<div class="xg-product-extract-header">
		<h1 class="title"><?php esc_html_e('Envato Purchase Verify By Purchase Code','irtech');?></h1>
		<p><?php esc_html_e('verify your envato customer using their purchase code','irtech');?></p>
	</div>
	<?php
		$irtech = Irtech();
    ...
    <div class="xg-notice-area <?php echo $response_status == 200 ? esc_attr('success') : 'danger'?>">
	    ...
	    foreach ($purchase_verify_info as $key){
	    	$message = Irtech()->get_flash_message($key);
	    	if (!empty($message)){
	...
	<form action="<?php echo admin_url('admin-post.php')?>" method="POST">
		...
		<input type="text" name="purchase_code" placeholder="<?php esc_html_e('Purchase Code','irtech');?>">
		<button type="submit" class="button button-primary "><?php esc_html_e('Purchase Verify','irtech');?></button>
	</form>
</div>