...
if (!class_exists('Irtech_Cron_Job')){
	class Irtech_Cron_Job{
		//$instance variable
		...
		public function __construct() {
			...
			add_action( 'irtech_cron_hook', array($this,'irtech_cron_exec') );
			add_filter( 'cron_schedules', array($this,'irtech_add_cron_interval') );
			if ( ! wp_next_scheduled( 'irtech_cron_hook' ) ) {
				wp_schedule_event( time(), 'hourly', 'irtech_cron_hook' );
			}
		}
		/**
		 * get Instance
		...
		public function irtech_cron_exec(){
			$query_args = array(
				'post_type' => 'download',
				'post_status' => 'publish',
				'posts_per_page' => -1
			...
			while ($all_products->have_posts()){
				...
				$post_id = get_the_ID();
				$envato_product_id = get_post_meta($post_id,'irtech_envato_product_id',true);
				$updated_at = get_post_meta($post_id,'irtech_updated_at',true);
		 ...
		 *
		 * */
		public function irtech_add_cron_interval( $schedules ) {
			$schedules['hourly'] = array(
				'interval' => 3600,
				'display'  => esc_html__( 'Every hour' ,'irtech-master'),
			);
		...
		public function update_product_data($post_id,$updated_data){
			...
			if (property_exists($updated_data,'previews') && !empty($post_id)){
				...
				if (property_exists($preview_images,'icon_preview')){
					...
					if (is_array($thumbnail_image_url)){
						...
						update_post_meta($post_id,'irtech_thumbnail',
							array(
				...
				if (property_exists($updated_data,'attributes')){
					...
					foreach ($item_attributes as $att){
						...
						if (in_array('columns',$attrb)){
							$irtech_product_info['columns'] = !empty($attrb['value']) ? $attrb['value'] : '';

						}else if (in_array('compatible-browsers',$attrb)){
							$irtech_product_info['compatible_browsers'] = !empty($attrb['value']) ? implode(' , ',$attrb['value']) : '';

						}else if (in_array('compatible-software',$attrb)){
							$irtech_product_info['compatible-software'] = !empty($attrb['value']) ? implode(' , ',$attrb['value']) : '';

						}else if (in_array('compatible-with',$attrb)){
							$irtech_product_info['compatible-with'] = !empty($attrb['value']) ? implode(' , ',$attrb['value']) : '';

						}else if (in_array('demo-url',$attrb)){
							$irtech_product_info['demo_url'] = !empty($attrb['value']) ? $attrb['value'] : '';

						}else if (in_array('documentation',$attrb)){
							$irtech_product_info['documentation'] = !empty($attrb['value']) ? $attrb['value'] : '';

						}else if (in_array('framework',$attrb)){
							$irtech_product_info['framework'] = !empty($attrb['value']) ? implode(' , ',$attrb['value']) : '';

						}else if (in_array('gutenberg-optimized',$attrb)){
							$irtech_product_info['gutenberg-optimized'] = !empty($attrb['value']) ? $attrb['value'] : '';

						}else if (in_array('high-resolution',$attrb)){
							$irtech_product_info['high_resolution'] = !empty($attrb['value']) ? $attrb['value'] : '';

						}else if (in_array('themeforest-files-included',$attrb)){
							$irtech_product_info['themeforest_files_included'] = !empty($attrb['value']) ? implode(' , ',$attrb['value']) : '';

						}else if (in_array('widget-ready',$attrb)){
							$irtech_product_info['widget_ready'] = !empty($attrb['value']) ? $attrb['value'] : '';
				...
				if (property_exists($updated_data,'attributes')) {
					...
					foreach ($item_attributes as $att){
						...
						if (in_array('columns',$attrb)){
							...
							$irtech_product_info['layout'] = !empty($attrb['value']) ? $attrb['value'] : '';
				...
				if (property_exists($updated_data,'attributes')) {
					...
					foreach ($item_attributes as $att){
						...
						if (in_array('compatible-software',$attrb)){
							...
							$irtech_product_info['graphics_files_included'] = !empty($attrb['value']) ? implode(' , ',$attrb['value']) : '';
					 ...
					 if (property_exists($body,'attributes')){
                        ...
                        foreach ($item_attributes as $att){
                           ...
                           if (in_array('compatible-browsers',$attrb)){
                                ...
                                $irtech_product_info['software_framework'] = !empty($attrb['value']) ? implode(' , ',$attrb['value']) : '';

                            }else if (in_array('source-files-included',$attrb)){
                                $irtech_product_info['source-files-included'] = !empty($attrb['value']) ? implode(' , ',$attrb['value']) : '';
			...
			if (!empty($post_id)){
				foreach ($irtech_product_info as $key => $value){
					update_post_meta($post_id,'irtech_'.$key,$value );
				}
			}
		}
