	...
	class Irtech_Envato{
		...
		public static function get_product_rating_by_id($item_id,$post_id){
			...
			if (is_array($response) && !is_wp_error($response)){
				...
				update_post_meta($post_id,'irtech_rating',$body->rating);
				update_post_meta($post_id,'irtech_rating_count',$body->rating_count);
		...
		public function extract_product_info_from_envato(){
			...
			if (is_array($response) && !is_wp_error($response)){
				$body = json_decode($response['body']);
				if (property_exists($body,'error')){
					Irtech()->set_flash_message('purchase_status',404);
					Irtech()->set_flash_message('purchase_message', $body->description);
				...
				if (property_exists($body,'previews') && !empty($post_id)){
					...
					if (property_exists($preview_images,'icon_preview')){
						...
						if (is_array($thumbnail_image_url)){
							...
							add_post_meta($post_id,'irtech_thumbnail',
								array(
				...
				if (!empty($post_id)){
					foreach ($irtech_product_info as $key => $value){
						add_post_meta($post_id,'irtech_'.$key,$value );
					}
				}

				Irtech()->set_flash_message('purchase_status',200);
				Irtech()->set_flash_message('extract_message',esc_html__('Product Extract & Imported Success'));
				wp_safe_redirect($redirect_path);
		...
		public function envato_purchase_verify(){
			...
			if (is_array($response) && !is_wp_error($response)){
				...
				$purchase_verify_info = [
					'purchase_message' => esc_html__('Purchase Verify Success, Purchase Details Below','irtech'),
					'buyer' => $body->buyer,
				...
				foreach ($purchase_verify_info as $key => $message){
					Irtech()->set_flash_message($key,$message);
				}
				wp_safe_redirect($redirect_path);
			}
