...
if (!class_exists('Irtech_Helper_Functions')) {

	class Irtech_Helper_Functions
	{
		/*
		* $instance
		* @since 1.0.0
		* */
		...
		function post_thumbnail() {
			...
			if ( is_singular() ) :
                ...
                <div class="post-thumbnail thumb">
					<?php the_post_thumbnail( 'irtech_classic' ); ?>
                </div><!-- .post-thumbnail -->
			...
			<?php else : ?>
                <div class="thumb">
                    <a class="post-thumbnail " href="<?php the_permalink(); ?>" aria-hidden="true" tabindex="-1">
						<?php
						the_post_thumbnail( 'irtech_classic', array(
		...
		public function link_pages(){
			...
			$defaults = array(
				'before' => '<div class="wp-link-pages"><span>'.esc_html__('Pages:' ,'irtech').'</span>',
				'after' => '</div>',
				'link_before' => '',
				'link_after' => '',
		...
		* */
		public function download_pagination( $nav_query = NULL ){
			...
			if( is_array($page_format) ) {
				...
				echo '<li><span>'. esc_html($paged) . esc_html__(' of ','irtech') . esc_html($nav_query->max_num_pages) .'</span></li>';
		...
		public function post_pagination( $nav_query = NULL ){
			...
			if(NULL == $nav_query){
				...
				if( is_array($page_format) ) {
					$paged = ( get_query_var('paged') == 0 ) ? 1 : get_query_var('paged');
					echo '<div class="blog-pagination margin-top-60"><ul>';
					echo '<li><span>'. esc_html($paged) . esc_html__(' of ','irtech') . esc_html($wp_query->max_num_pages) .'</span></li>';
					foreach ( $page_format as $page ) {
						echo "<li>".wp_kses_post($page)."</li>";
					}
					echo '</ul></div>';
		...
		public function posted_by() {
			...
			/* translators: %s: post author. */
				esc_html_x( ' %s', 'post author', 'irtech' ),
				'<span class="author vcard"><a class="url fn n" href="' . esc_url( get_author_posts_url( get_the_author_meta( 'ID' ) ) ) . '"><i class="fas fa-user" aria-hidden="true"></i> '. esc_html__('By ','irtech'). esc_html( get_the_author() ) . '</a></span>'
			);
		...
		* */
		public function posted_tag(){
			...
			$tags_list = get_the_tag_list( '', esc_html_x( ' ', 'list item separator', 'irtech' ) );
			if ( $tags_list ) {
				/* translators: 1: list of tags. */
				printf( '<ul class="tags"><li class="title">' . esc_html__( 'Tags: ', 'irtech' ) . '</li><li>' . ' %1$s'. '</li></ul>', $tags_list ); // WPCS: XSS OK.
			}
		}
		/**
		 * The post navigation
		...
		public function post_navigation(){
			the_post_navigation( array(
				'prev_text'                  => '<i class="fa fa-angle-double-left" aria-hidden="true"></i>&nbsp;'.esc_html__( 'Prev Post','irtech' ),
				'next_text'                  => esc_html__( 'Next Post','irtech' ).'&nbsp;<i class="fa fa-angle-double-right" aria-hidden="true"></i>',
			) );
		 ...
		 * @sicne 1.0.0
		 * */
		public function is_irtech_active(){
			$theme_name_array = array('Irtech','Irtech Child');
			$current_theme = wp_get_theme();
			$current_theme_name = $current_theme->get( 'Name' );
			return in_array($current_theme_name,$theme_name_array) ? true : false;
		}
		/**
		 * is irtech master active
		 * @sicne 1.0.0
		 * */
		public function is_irtech_master_active(){
			return defined('IRTECH_MASTER_SELF_PATH') ? true : false;
		}
	...
	if (class_exists('Irtech_Helper_Functions')){
		Irtech_Helper_Functions::getInstance();
	}
}
