...
if ( !class_exists('Irtech_Master_shortcodes') ){

	class Irtech_Master_shortcodes{
		...
		* */
		public function __construct() {
			//social post share
			add_shortcode('irtech_post_share',array($this,'post_share'));

			//info_item
			add_shortcode('irtech_info_item_wrap',array(__CLASS__,'info_item_wrap'));
			add_shortcode('irtech_info_link',array(__CLASS__,'info_link'));
			add_shortcode('irtech_info_inline_text',array(__CLASS__,'info_inline_text'));

			//info item two
			add_shortcode('irtech_info_item_two_wrap',array(__CLASS__,'info_item_two_wrap'));
			add_shortcode('irtech_info_item_two',array(__CLASS__,'info_item_two'));
			//info item two
			add_shortcode('irtech_info_text_wrap',array(__CLASS__,'info_text_wrap'));
			add_shortcode('irtech_info_text_item',array(__CLASS__,'info_text_item'));
		}
	   ...
	   * */
		...
		public static function post_share($atts, $content = null){
			...
			if ( is_singular() || is_home() ){
				...
				$irtech_url = urlencode_deep(get_permalink());
				//get current page title
				$irtech_title = str_replace(' ','%20',get_the_title($post->ID));
				//get post thumbnail for pinterest
				$irtech_thumbnail = wp_get_attachment_image_src(get_post_thumbnail_id($post->ID),'full');
				$irtech_thumbnail_url = is_array($irtech_thumbnail) && !empty($irtech_thumbnail[0]) ? $irtech_thumbnail[0] : '';
				//all social share link generate
				$facebook_share_link = 'https://www.facebook.com/sharer/sharer.php?u='.$irtech_url;
				$twitter_share_link = 'https://twitter.com/intent/tweet?text='.$irtech_title.'&amp;url='.$irtech_url.'&amp;via=Crunchify';
				$linkedin_share_link = 'https://www.linkedin.com/shareArticle?mini=true&url='.$irtech_url.'&amp;title='.$irtech_title;
				$pinterest_share_link = 'https://pinterest.com/pin/create/button/?url='.$irtech_url.'&amp;media='.$irtech_thumbnail_url.'&amp;description='.$irtech_title;

                $output .='<ul class="social-share">';
                $output .='<li class="title">'.esc_html__('Share:','irtech').'</li>';
	...
	if ( class_exists('Irtech_Master_shortcodes') ){
		Irtech_Master_shortcodes::getInstance();
	}
