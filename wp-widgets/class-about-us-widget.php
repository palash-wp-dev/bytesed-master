...
class Irtech_About_Us_Widget extends WP_Widget{

	public function __construct() {
		parent::__construct(
			'irtech_about_us',
			esc_html__('Irtech: About Us','irtech-master'),
			array('description' => esc_html__('Display about us widget, with an image and social links','irtech-master'))
		);
	}
	...
	public function form($instance){
		...
		?>
		<p class="irtech_sub_title"><?php esc_html_e('About Us Logo','irtech-master')?></p>
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('bf_logo')); ?>"></label>
			<input type="hidden" class="widefat irtech_logo_id"
			...
			/>
		<div class="irtech-logo-preview"></div>
		<input type="button" class="irtech_flogo_uploader" value="<?php esc_html_e('Upload Logo','irtech-master');?>">
		</p>
		<p>
			<label for="<?php echo esc_attr($this->get_field_name('bf_description'))?>"><?php esc_html_e('About Widget Description','irtech-master')?></label>
			<textarea name="<?php echo esc_attr($this->get_field_name('bf_description'))?>" id="<?php echo esc_attr($this->get_field_id('bf_description'))?>" cols="30" class="irtech-form-control" rows="5"><?php echo esc_html($instance['bf_description'])?></textarea>
		</p>
		<?php foreach ( $social_icons as $sci ) : ?>
            <p>
                <label for="<?php echo esc_attr($this->get_field_id( $sci ) ); ?>"><?php echo esc_html( ucfirst( $sci ) . " " . esc_html__( 'URL', 'irtech-master' ) ); ?>
                    : </label>
                <br/>
                ...
                <small><?php echo esc_html__('Leave it blank if you don\'t want this social icon','irtech-master')?></small>
            </p>
		<?php
		endforeach;
	}
...
if (!function_exists('Irtech_About_Us_Widget')){
	function Irtech_About_Us_Widget(){
		register_widget('Irtech_About_Us_Widget');
	}
	add_action('widgets_init','Irtech_About_Us_Widget');
}
