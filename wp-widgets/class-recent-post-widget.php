...
class Irtech_Recent_Post_Widget extends WP_Widget {

	public function __construct() {
		parent::__construct(
			'irtech_popular_posts',
			esc_html__( 'Irtech: Recent Posts with Thumbnails', 'irtech-master' ),
			array( 'description' => esc_html__( 'Display your recent posts, with a thumbnail.', 'irtech-master' ) )
		);
	}
	...
	public function widget( $args, $instance ) {
		...
		if ( $recent_articles->have_posts() ):?>
			...
			esc_html__( ' Oops, there are no posts.', 'irtech-master' )
			?>
	...
	public function form( $instance ) {
		...
		if ( ! empty( $instance ) && $instance['title'] ) {
			...
			$title = esc_html__( 'Recent Posts', 'irtech-master' );
		}
		...
		$order_arr   = array(
			'ASC'  => esc_html__( 'Acceding', 'irtech-master' ),
		...
		$orderby_arr = array(
			...
			'comment_count' => esc_html__( 'Most Comment', 'irtech-master' )
		);
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'no_of_posts' ) ) ?>"><?php esc_html_e( 'No Of Posts', 'irtech-master' ); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'order' ) ) ?>"><?php esc_html_e( 'Order', 'irtech-master' ); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'orderby' ) ) ?>"><?php esc_html_e( 'OrderBy', 'irtech-master' ); ?></label>
...
if ( ! function_exists( 'Irtech_Recent_Post_Widget' ) ) {
	function Irtech_Recent_Post_Widget() {
		register_widget( 'Irtech_Recent_Post_Widget' );
	}

	add_action( 'widgets_init', 'Irtech_Recent_Post_Widget' );
}